...
def compute_evm(df: pd.DataFrame, current_day: datetime = None):
    ...
    # Compute PV, EV, AC, SV%, and CPI for each date
    pv_ev_list = []
    actual_cost_list = []
    sv_pct_list = []
    cpi_list = []
    ...
    # Filter SV% and CPI to dates up to latest_done_date
    sv_pct_filtered = []
    cpi_filtered = []
    ...
    filtered_planned = [(date.strftime('%Y-%m-%d'), pv) for date, pv, _ in pv_ev_list if date.strftime('%Y-%m-%d') in milestone_dates]
    filtered_earned = [(date.strftime('%Y-%m-%d'), ev) for date, _, ev in pv_ev_list if date.strftime('%Y-%m-%d') in milestone_dates]
    filtered_actual = [(date.strftime('%Y-%m-%d'), cost) for date, cost in actual_cost_list if date.strftime('%Y-%m-%d') in milestone_dates]
